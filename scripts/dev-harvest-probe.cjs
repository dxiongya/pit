// dev-harvest-probe.cjs — standalone accuracy probe for the source-level
// design harvester. Loads a real site in a hidden BrowserWindow and runs the
// in-page HARVEST_SCRIPT, printing the structured SourceFacts as JSON.
//
//   npx electron scripts/dev-harvest-probe.cjs https://linear.app
//
// Once the output looks good, the HARVEST_SCRIPT body moves to src/main/harvest.ts.

const { app, BrowserWindow } = require('electron')

// Self-contained in-page expression. MUST be a single IIFE returning a
// JSON-serialisable object (no closures over outer scope). Mirrors the
// PREP_SCRIPT/DISCOVER_SCRIPT style in capture.ts.
const HARVEST_SCRIPT = `(() => {
  ...
  // ---- logo: dig out the REAL mark (inline SVG markup / <img> URL), not just a flag ----
  const pickLogo = () => {
    const out = { svg: null, img: null, source: null }
    // Strongest → weakest signals. Home-link + logo-named scopes first so we
    // don't grab a hamburger/utility icon from the header.
    const scopes = [
  ...
  const logo = {
    ...pickLogo(),
    // high-quality icon fallbacks (often the brand mark when there's no inline logo)
  ...
  const colorTokens = Object.entries(tokens).filter(([, v]) => /#|rgb|hsl/.test(v)).length
  return {
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await new Promise((r) => setTimeout(r, 2800)) // settle SPA hydration + fonts
    const result = await win.webContents.executeJavaScript(HARVEST_SCRIPT, true)
    process.stdout.write(JSON.stringify(result, null, 2) + '\n')
  } catch (e) {
    process.stderr.write('PROBE ERROR: ' + (e && e.message) + '\n')
  } finally {
    app.quit()
  }
})
